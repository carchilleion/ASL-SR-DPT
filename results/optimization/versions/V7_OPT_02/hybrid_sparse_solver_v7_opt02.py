"""
ASL-SR-DPT Solver - Version V7_OPT_02
Optimization Candidate J: Direct BLAS Euclidean Norm Computation

Replaces generic numpy.linalg.norm Python wrapper dispatches in the inner loop
with direct BLAS dot products: sqrt(dot(dz, dz)) / (sqrt(dot(z, z)) + 1e-8).
Preserves exact double-precision IEEE-754 floating point arithmetic.
"""
...
class HybridSparseSolverV7Optimized:
    """
    ASL-SR-DPT Sparse Recovery Solver - V7_OPT_02.
    Candidate J: Direct BLAS Euclidean Norm computation in convergence check.
    """
    ...
    def denoise_patch(
        ...
        """
        Recover a sparse coefficient vector from a measurement vector using V7_OPT_02.
        """
        ...
        if detailed_profile:
            ...
            initial_state = {
                ...
                "initial_residual": float(np.sqrt(np.dot(r, r))),
                "initial_coefficient_norm": float(np.sqrt(np.dot(z, z))),
        ...
        for iteration in range(max_iter):
            # 1. Active support determination
            if detailed_profile:
                t_s = time.perf_counter()
            ...
            if iteration % support_reopen_interval == 0:
                ...
                if not np.any(active_indices):
                    ...
                    all_active = bool(np.all(active_indices))
            ...
            if not line_search_success:
                ...
                if detailed_profile:
                    cur_res = float(np.sqrt(np.dot(r, r)))
            ...
            # Candidate J: Direct BLAS Euclidean norm computation
            diff_z = z_new - z
            norm_dz = np.sqrt(np.dot(diff_z, diff_z))
        ...
        if detailed_profile:
            t_s = time.perf_counter()
        final_residual_norm = float(np.sqrt(np.dot(self.A @ z - y, self.A @ z - y)))
        if detailed_profile:
            t_residual_total += time.perf_counter() - t_s
