"""
ASL-SR-DPT Solver - Version V7_OPT_06
Optimization Candidate L: Fast Primary Candidate Path in Line Search

Specializes the line search for the primary Armijo candidate (mu):
Since >95% of iterations accept on the initial step size mu without backtracking,
evaluating the primary candidate before entering the loop eliminates loop generator
instantiation, backtrack iteration counting, and branch condition checks on >95% of steps.
If the primary candidate is rejected, execution falls back seamlessly to the full
backtracking loop with midpoint acceleration.
Mathematical equivalence: 100% bit-for-bit identical decisions and outputs.
"""
...
class HybridSparseSolverV7Optimized:
    """
    ASL-SR-DPT Sparse Recovery Solver - V7_OPT_06.
    Candidate L: Fast Primary Candidate Path in Line Search.
    """
    ...
    def denoise_patch(
        ...
        """
        Recover a sparse coefficient vector from a measurement vector using V7_OPT_06.
        """
        ...
        if detailed_profile:
            ...
            neg_inv_2sigma2_0 = -0.5 / (sigma * sigma)
            w0 = np.exp((z * z) * neg_inv_2sigma2_0)
            initial_state = {
                "initial_objective": fid - self.lambda_reg * float(w0.sum()),
                "initial_fidelity": fid,
                "initial_sparsity": -self.lambda_reg * float(w0.sum()),
        ...
        for iteration in range(max_iter):
            ...
            step_size_initial = mu
            used_midpoint = False
            backtrack_iter = 1
            if detailed_profile:
                count_backtracks += 1

            # Candidate L: Fast primary candidate evaluation (bt=0)
            r_cand = r + mu * A_d
            fid_cand = 0.5 * float(np.dot(r_cand, r_cand))

            z_candidate_active = z_active + mu * d_active
            if all_active:
            ...
            # Primary Armijo test
            if cost_candidate <= cost_current + armijo_c * mu * grad_dot_d:
                ...
                # Fallback: Midpoint or Backtracking loop
                line_search_success = False
                best_z_active = None
                step_size = mu

                # Check initial midpoint first if enabled
                if use_midpoint:
