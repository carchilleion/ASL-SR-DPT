"""
ASL-SR-DPT Solver - Version V7_OPT_01
Optimization Candidate M: Diagnostic & Telemetry Overhead Elimination

Eliminates diagnostic dictionary allocation, list instantiations, and per-iteration
list append operations when return_diagnostics=False. When return_diagnostics=True,
maintains full telemetry for 100% backward compatibility with unit tests.
"""
...
class HybridSparseSolverV7Optimized:
    """
    ASL-SR-DPT Sparse Recovery Solver - V7_OPT_01.
    Candidate M: Diagnostic overhead elimination when return_diagnostics=False.
    """
    ...
    def denoise_patch(
        ...
        """
        Recover a sparse coefficient vector from a measurement vector using V7_OPT_01.
        """
        ...
        if return_diagnostics:
            ...
            if iter_count > 0:
                diagnostics["mean_active_count"] = float(sum_active_count) / float(iter_count)
                diagnostics["active_support_ratio"] = float(sum_active_count) / float(iter_count * self.N)
            ...
            if detailed_profile:
                ...
                diagnostics["operation_counts"] = {
                    ...
                    "failed_line_searches": failed_ls_count,
