...
def solve_patches(
    ...
    normalized_residuals_list = []

    for i in iterator:
        y_meas = measurements[i]
        dc_val = dc_coefficients[i] if dc_coefficients is not None else None
        y_norm = max(float(np.linalg.norm(y_meas)), 1e-8)
        dim_m = float(y_meas.size)
        ...
        if solver_name.startswith("ASL-SR-DPT"):
            ...
            solve_fn = getattr(solver, "denoise_patch", getattr(solver, "solve", None))
            theta_hat, diag = solve_fn(
                y_meas,
            ...
            r_val = float(diag["final_residual"])
            residuals_list.append(r_val)
            relative_residuals_list.append(r_val / y_norm)
            normalized_residuals_list.append(r_val / np.sqrt(dim_m))
            final_sigmas.append(diag["final_sigma"])
            active_ratios.append(diag["active_support_ratio"])
            mean_active_counts.append(diag["mean_active_count"])
            failed_ls_list.append(diag["failed_line_searches"])
            accepted_steps_list.append(diag["accepted_steps"])
            admm_primal_list.append(0.0)
            admm_dual_list.append(0.0)
        ...
        # -------------------------------------------------------------
            ...
            solve_times.append(t1 - t0)
            iterations_list.append(diag["iterations"])
            # Record true measurement residual ||Az - y||_2 in residuals_list
            meas_res = float(diag.get("final_measurement_residual", diag.get("final_residual", float(np.linalg.norm(sensing_matrix @ z_hat - y_meas)))))
            residuals_list.append(meas_res)
            relative_residuals_list.append(meas_res / y_norm)
            normalized_residuals_list.append(meas_res / np.sqrt(dim_m))
            admm_primal_list.append(float(diag.get("final_primal_residual", 0.0)))
            admm_dual_list.append(float(diag.get("final_dual_residual", 0.0)))
            final_sigmas.append(0.0)
    ...
    mean_relative_residual = float(np.mean(relative_residuals_list)) if relative_residuals_list else 0.0
    mean_normalized_residual = float(np.mean(normalized_residuals_list)) if normalized_residuals_list else 0.0
    ...
    summary_stats = {
        ...
        "relative_measurement_residual": mean_relative_residual,
        "normalized_measurement_residual": mean_normalized_residual,
        "admm_primal_residual": float(np.mean(admm_primal_list)) if any(admm_primal_list) else 0.0,
        "admm_dual_residual": float(np.mean(admm_dual_list)) if any(admm_dual_list) else 0.0,
        ...
        for noise_sigma in noise_levels:
            for img_idx, img_id in enumerate(image_ids):
                ...
                for solver_name in pending_solvers:
                    ...
                    is_dc = (sensing_mode == "dc")
                    m_res = stats.get("measurement_residual", stats["residual"])
                    rel_res = stats.get("relative_measurement_residual", 0.0)
                    norm_res = stats.get("normalized_measurement_residual", 0.0)
                    ...
                    row_data = {
                        ...
                        "measurement_residual": round(m_res, 6),
                        "relative_measurement_residual": round(rel_res, 6),
                        "normalized_measurement_residual": round(norm_res, 6),
                        "measurement_residual_ac": round(m_res, 6) if is_dc else "",
                        "relative_measurement_residual_ac": round(rel_res, 6) if is_dc else "",
                        "normalized_measurement_residual_ac": round(norm_res, 6) if is_dc else "",
                        "admm_primal_residual": round(stats.get("admm_primal_residual", 0.0), 6),
                        "admm_dual_residual": round(stats.get("admm_dual_residual", 0.0), 6),
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="ASL-SR-DPT solver version: 'baseline' (frozen V7) or 'optimized' (equivalent fast-paths)."
    )
