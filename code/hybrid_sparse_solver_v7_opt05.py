"""
ASL-SR-DPT Solver - Version V7_OPT_05
Optimization Candidate K: Vectorized Exponential Evaluation & Direct C-Method Sum

Optimizes the exponential surrogate evaluation:
1. Replaces unary minus allocation -(z*z)*inv with direct fused scalar factor:
   neg_inv_2sigma2 = -0.5 / (sigma * sigma), exponent = (z * z) * neg_inv_2sigma2.
2. Replaces generic Python dispatcher np.sum(w) with direct C ndarray method w.sum().
   Eliminates >155,000 Python function call dispatches per 500 patches.
3. Preserves exact IEEE-754 bit-for-bit floating-point equivalence.
"""
...
class HybridSparseSolverV7Optimized:
    """
    ASL-SR-DPT Sparse Recovery Solver - V7_OPT_05.
    Candidate K: Vectorized Exponential Evaluation & Direct C-Method Sum.
    """
    ...
    def denoise_patch(
        ...
        """
        Recover a sparse coefficient vector from a measurement vector using V7_OPT_05.
        """
        ...
        for iteration in range(max_iter):
            ...
            # 2. Shared evaluation: compute w = exp(-z^2/(2*sigma^2)) once using direct C-methods
            if detailed_profile:
                t_s = time.perf_counter()
            neg_inv_2sigma2 = -0.5 / (sigma * sigma)
